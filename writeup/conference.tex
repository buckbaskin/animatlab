...
\caption{Negative Velocity Testing (Reference in orange)}
\label{fig:TestVelNeg}
\end{figure}

Within the neuron network, acceleration is calculated primarily from actuator
pressures. The output is plotted compared with expected output. See \myref{fig:TestAccelPos}.

\begin{figure}
\centering
\includegraphics[width=3in]{results/TestAccelPos}
\caption{Positive Acceleration Tests (Reference in orange)}
...
% TODO*buckbaskin): Figure of neuron controller system model test
The system modeling network was tested as a complete network. See \myref{fig:SystemModel}.
% TODO*buckbaskin): Figure of neuron controller torque optimization test
\begin{figure}
\centering
\includegraphics[width=3in]{results/TestAccelPos}
...
% TODO(buckbaskin): Figure of neuron controller overall in step tests (fixed
% estimators)
