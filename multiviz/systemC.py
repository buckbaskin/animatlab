...
edges = {
    '_nte mult': {
        'neg prediction err': 'signal multiplier',
    },
    '_pte mult': {
        'pos prediction err': 'signal multiplier',
    },
    '-mt': {
        '_nte mult': 'signal multiplier',
        'inertia (test)': 'signal transfer',
    },
    '+mt': {
        '_pte mult': 'signal multiplier',
        'inertia (test)': 'signal transfer',
    },
    'abs vel': {
        'pos vel': 'signal transfer',
        'neg vel': 'signal transfer',
    },
    '-mt/v': {
        '-mt': 'signal transfer',
        'abs vel': 'signal divider',
    },
    '+mt/v': {
        '+mt': 'signal transfer',
        'abs vel': 'signal divider',
    ...
    'neg n delta': {
        'neg lambda': 'signal transfer',
    },
    'pos n delta': {
        'pos lambda': 'signal transfer',
    },
    'pos vel mult': {
        'pos vel': 'signal multiplier',
    },
    'neg vel mult': {
        'neg vel': 'signal multiplier',
    },
    'c++': {
        'pos lambda': 'signal transfer',
        'pos vel mult': 'signal multiplier',
    },
    'c--': {
        'neg lambda': 'signal transfer',
        'neg vel mult': 'signal multiplier',
    },
    'c+-': {
        'pos lambda': 'signal transfer',
        'neg vel mult': 'signal multiplier',
    ...
    'theta future': {
        'theta (test)': 'signal transfer',
        'pos vel mod': 'signal red 0.2x',
        'neg vel mod': 'signal inv red 0.2x',
    },
    'theta pos err': {
        'theta des (test)': 'signal transfer',
        'theta future': 'signal inverter',
    },
    'theta neg err': {
        'theta des (test)': 'signal inverter',
        'theta future': 'signal transfer',
    },
...
def reference_updateC(inputs, output):
    for input_ in inputs:
        if input_[0] == 'pos vel':
            vel_mV = input_[1]
        if input_[0] == 'pos prediction err':
            err_mV = input_[1]
    ...
    def updateN(err_mV, vel_mV):
        ...
        return (err_mV) * (1 / (1 + vel_mV))
    return (err_mV - updateN(err_mV, vel_mV)) / 5
...
if __name__ == '__main__':
    ...
    for iteration, input_combo in enumerate(input_combos):
        ...
        data_ref[iteration, -1] = reference_updateC(input_combo, output_neuron)
    ...
    # plt.legend()
    # plt.tight_layout()
    plt.savefig('images/results/New_SystemC.png')
    plt.show()